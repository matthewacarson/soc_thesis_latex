...
\usepackage[orientation=landscape,size=custom,width=11,height=8.5,scale=1,debug]{beamerposter}
...
This is \emph{not} necessarily the case.
	...
	\end{itemize}
The UA \emph{does} use pre-hire agreements, while the OCAW/USW \emph{does not}.
\end{frame}
...
But the international union has maintained a craft union orientation like the building trades.
	\begin{itemize}
...
The key distinction between these unions is the way in which they organize workers.
  \begin{itemize}
  	\item The Building Trades and OCAW/USW have work in the same industry.
  	\begin{itemize}
  		\item Building Trades: industry-friendly.
  		\item OCAW/USW: supports a move away from dirty energy.
  	\end{itemize}
  	\item The Machnists (craft union) supports social wage policy (single-payer healthcare), while the Building Trades do \\emph{not}.
  	\begin{itemize}
  		\item They are both craft unions.
  		\item But the Machinists have to organize via the industrial organizing path.
  	\end{itemize}
  \end{itemize}
\end{frame}
